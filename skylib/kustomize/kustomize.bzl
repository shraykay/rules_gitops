...
{}
    ...
    for inattr in ctx.attr.srcs:
        ...
        for infile in inattr.files.to_list():
            statements += ("echo $TARGET_DIR/{gitops_path}/{cluster}/{app_name}/{file}\n" +
                           "mkdir -p $TARGET_DIR/{gitops_path}/{cluster}/{app_name}\n" +
                           "echo '# GENERATED BY {rulename} -> {gitopsrulename}' > $TARGET_DIR/{gitops_path}/{cluster}/{app_name}/{file}\n" +
                           "{template_engine} --template={infile} --variable=NAMESPACE={namespace} --stamp_info_file={info_file} >> $TARGET_DIR/{gitops_path}/{cluster}/{app_name}/{file}\n").format(
